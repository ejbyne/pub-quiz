...
describe('answer sheet reducer', () => {
  ...
  describe('answers', () => {
    ...
    it("stores multiple player's answers for a round", () => {
      ...
      const resultAfterSecondAnswer = answerSheetReducer(
        resultAfterFirstAnswer,
        {
          type: 'AnswerChanged',
          payload: {
            roundNumber: 0,
    ...
    it('handles empty answers', () => {
      ...
      expect(result).toEqual({
        rounds: [
          undefined,
          [undefined, { answer: 'My answer for the second question' }],
        ],
      });
    });
  });
  ...
  describe('marks', () => {
    it("marks a player's answer", () => {
      const answerSheet = { rounds: [[{ answer: 'My first answer' }]] };
      ...
      expect(result).toEqual({
        rounds: [[{ answer: 'My first answer', mark: 1 }]],
      });
    });
  });
});
